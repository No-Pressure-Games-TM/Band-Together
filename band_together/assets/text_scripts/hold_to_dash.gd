...
func update_text(is_using_controller: bool):
	clear()
	if is_using_controller:
		append_text("While dashing, hold [img]%s[/img]/[img]%s[/img] to continue." % [left_gamepad, right_gamepad])
	else:
		append_text("While dashing, hold [img]%s[/img]/[img]%s[/img] to continue." % [left, right])
